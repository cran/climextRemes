...
#' @param upperTail logical value that indicates whether values of \code{y} are upper (right) tail values (TRUE) or lower (left) tail values (FALSE)
#'
#' @export
#'
remove_runs <- function(y, index, upperTail = TRUE){
    ...
    if(upperTail) FUN <- max else FUN <- min
    if(n > 1){
        ...
        for(start in seqStarts){
            ...
            tmp <- y[start:pos]
            jitter <- stats::rnorm(length(tmp), 0, 1e-12)  # amounts to randomly choosing a max when values are equal
...
screen_within_block <-  function(y, index, blockLength = 10){
    ...
    if(n > 1){
        ...
        while(i < length(y)){
            ...
            if(i > start){
                jitter <- stats::rnorm(i - start + 1, 0, 1e-12)
                tmp <- y[start:i]
